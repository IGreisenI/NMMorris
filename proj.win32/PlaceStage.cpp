...
void PlaceStage::handleInput(cocos2d::Touch* touch, cocos2d::Event* event, Player* _activePlayer) {
    ...
    PhysicsRayCastCallbackFunc callback = [&](PhysicsWorld& world, const PhysicsRayCastInfo& info, void* data) -> bool {
        ...
        if (node && dynamic_cast<Piece*>(node) && !dynamic_cast<Piece*>(node)->isPlaced() && _activePlayer->isPlayerPiece(dynamic_cast<Piece*>(node))) {
...
void PlaceStage::handlePieceTouch(Piece* touchedPiece, Player* _activePlayer)
{
    if (_selectedPiece) {
...
void PlaceStage::handleSpotTouch(Spot* touchedSpot)
{
    Spot* parentSpot = dynamic_cast<Spot*>(_selectedPiece->getParent());
    if (parentSpot) {
